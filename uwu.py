...
# ── PATCH settings.json (toggle atau update nilai) ───────────
...
def update_settings():
    ...
    try:
        payload = request.get_json()
        # payload: {"path": ["commands","hunt","enabled"], "value": false}
        key_path = payload["path"]
        new_value = payload["value"]
...
def get_global_settings():
    if not check_password(request):
        return "Invalid Password", 401
    try:
        data = read_json("config/global_settings.json")
        # Jangan expose password ke frontend
        safe = json.loads(json.dumps(data))
        safe["website"]["password"] = "••••••••"
        return jsonify({"status": "success", "data": safe})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500


# ── PATCH global_settings.json ───────────────────────────────
@app.route("/api/global_settings", methods=["PATCH"])
def update_global_settings():
    ...
    try:
        ...
        # Blokir update password via dashboard
        if key_path == ["website", "password"]:
            return jsonify({"status": "error", "message": "Cannot change password via dashboard"}), 403
        ...
        write_json("config/global_settings.json", data, global_settings_lock)

        # Reload global_settings_dict in-memory juga
        global global_settings_dict
        global_settings_dict = read_json("config/global_settings.json")
...
# ── GET channelSwitcher users (untuk editor Channel IDs) ──────
...
# ── PATCH channels — update channelSwitcher users array ──────
@app.route("/api/channels", methods=["PATCH"])
def update_channels():
    if not check_password(request):
        return "Invalid Password", 401
    try:
        payload = request.get_json()
        # payload: {"channelSwitcher": {...}, "webhookUrl": "..."}

        data = read_json("config/global_settings.json")

        if "channelSwitcher" in payload:
            data["channelSwitcher"] = payload["channelSwitcher"]
        if "webhookUrl" in payload:
            data["webhook"]["webhookUrl"] = payload["webhookUrl"]
        if "webhookEnabled" in payload:
            data["webhook"]["enabled"] = payload["webhookEnabled"]
